...
type VarOrdUnifierPair c = ([LVar], [(LVar, Int)], [LSubstVFresh c], [LSubstVFresh c])
...
solveDisjointSystemsWithPartition sortOf sys (unifierL, unifierR) vars (vIndex:varIndexes) = let
  ...
  then Just (corrP, vIndex, map substFromListVFresh solvedSysL'', map substFromListVFresh solvedSysR'')
...
cleanSolvedSystem orgVars (varOrd, varInd, substL, substR) = let
    substlistL = map substToListVFresh substL
    substlistR = map substToListVFresh substR
    allVarsL = map fst (concat substlistL) ++ concatMap (varsVTerm . snd) (concat substlistL)
    newVarsR = filter (`notElem` orgVars) $
      map fst (concat substlistR) ++ concatMap (varsVTerm . snd) (concat substlistR)
    ...
    substR' = map (map (bimap (varSwap newVarsRSubst) (applyVTerm newVarsRSubst'))) substlistR
  in (varOrd, varInd, substL, map substFromListVFresh substR')
  where
