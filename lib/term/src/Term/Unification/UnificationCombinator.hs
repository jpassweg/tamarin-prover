...
matchHomACCLTerm sortOf mhnd matchProblem = case flattenMatch matchProblem of
  ...
  Just ms -> let
      ...
      unifier' = map (map (bimap (varSwap2 newVarsSubst) (applyVTerm newVarsSubst'))) unifier
    in map (substFromList . map (second fromMConst)) unifier'

varSwap2 :: [(LVar,LVar)] -> LVar -> LVar
varSwap2 vS v = foldl (\v1 (v2,v3) -> if v1 == v2 then v3 else v1) v vS
